...
const EditSubHeading = () => {
  const [subheading, setHeading] = useState({ name: "", firstCategoryId: {} });
  ...
  const fetchSubHeading = async () => {
    if (id) {
      try {
        ...
        const subHeadingData = res.data;

        // Find the full firstCategory object from mainCategories if needed
        const matchedCategory = mainCategories.find(
          (cat) => cat._id === subHeadingData.firstCategoryId
        );

        setHeading({
          ...subHeadingData,
          firstCategoryId: matchedCategory || {
            _id: subHeadingData.firstCategoryId,
          },
        });
      } catch (error) {
        Swal.fire({
  ...
  const handleMainCategoryChange = (e) => {
    ...
    setHeading((prev) => ({
      ...prev,
      firstCategoryId: selectedCategory,
    }));
  };
  ...
  const onClickHandle = async (e) => {
    ...
    try {
      const payload = {
        ...
        firstCategoryId: subheading.firstCategoryId._id,
      };
      ...
      <form>
        ...
        <div className="mt-5">
          ...
          <div className="mt-2">
            <select
              name="mainCategoryId"
              value={subheading.firstCategoryId?._id || ""}
